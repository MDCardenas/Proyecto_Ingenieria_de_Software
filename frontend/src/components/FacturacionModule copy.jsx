...
export default function Facturacion({ onCancel }) {
  ...
  const [productosStock, setProductosStock] = useState([]); // Nuevo estado para productos
  ...
  const cargarDatosIniciales = async () => {
    try {
      ...
      const endpoints = {
        ...
        productos: 'http://localhost:8000/api/joyas/' // Nueva endpoint para productos
      };
  ...
  // validarCampos, limpiarError, guardarFacturaEnBD, generarPDF, etc.
  ...
  const generarPDF = async () => {
    ...
    try {
      ...
      if (resultadoBD) {
        ...
        alert(`Factura generada exitosamente! (No se pudo guardar en la base de datos)`);
      }
